...
def process_mc():
    ...
    if local_mc_tags.exists():
        for filename in os.listdir(local_mc_tags):
            if filename.endswith(".png"):
                shutil.copy(local_mc_tags / filename, mc_assets_dir / filename)
        print("[鸣潮] 成功复制本地鸣潮图标资产。")
    else:
        print("警告: 未找到本地鸣潮 tags 图标文件夹，跳过资产复制。")
